...
- **Symptom**: Calling `https://zap-gateway-b9ei3tej.uc.gateway.dev/api/Auth/check-account` returned a 404 "The current request is not defined by this API" error.
- **Root Cause**: The physical API gateway (`zap-gateway`) was still running an older configuration that lacked the newly added `/api/Auth/check-account` and `/api/Auth/social-login` endpoints. Requests were rejected at the gateway level before reaching the Identity API backend. Furthermore, the gateway ID was `zap-api` string, not `zap-identity-api` as assumed initially.
- **Resolution**:
...
- **Symptom**: The new C# logic governing Social Logins and unified Account Checks exists locally but has **not** been successfully deployed to the Cloud Run service (`zap-identity-api`).
- **Root Cause**: 
  ...
  - Manual deployment attempts via `gcloud builds submit` under `linh.nguyen@zap.vn` failed due to missing Google Cloud IAM permissions:
